...
MEMORY
{
    ROM (rx) : ORIGIN = (0 + 0x0), LENGTH = (458752 - 0x0)
...
ztest :
{
 ...
 intc_table_area : SUBALIGN(4) { _intc_table_list_start = .; KEEP(*(SORT_BY_NAME(._intc_table.static.*))); _intc_table_list_end = .; } > ROM
...
flashBlockDesc_area : ALIGN_WITH_INPUT { _flashBlockDesc_list_start = .; KEEP(*(SORT_BY_NAME(._flashBlockDesc.static.*))); _flashBlockDesc_list_end = .; } > RAM AT > ROM
usbd_context_area : ALIGN_WITH_INPUT { _usbd_context_list_start = .; KEEP(*(SORT_BY_NAME(._usbd_context.static.*))); _usbd_context_list_end = .; } > RAM AT > ROM
usbd_class_fs_area : ALIGN_WITH_INPUT { _usbd_class_fs_list_start = .; KEEP(*(SORT_BY_NAME(._usbd_class_fs.static.*))); _usbd_class_fs_list_end = .; } > RAM AT > ROM
usbd_class_hs_area : ALIGN_WITH_INPUT { _usbd_class_hs_list_start = .; KEEP(*(SORT_BY_NAME(._usbd_class_hs.static.*))); _usbd_class_hs_list_end = .; } > RAM AT > ROM
    __data_region_end = .;
 __kernel_ram_end = .;
